        ...
        searchUsers: async (_, { key }: { key: string }, { loggedInUser }): Promise<User[]> => {
            try {
                return mapUser(await client.user.findMany({
                    ...
                    take: 50,
                    select: { username: true, account: true, avatarUrl: true, follower: { where: { account: loggedInUser }, select: { account: true } }, followReqToMe: { where: { account: loggedInUser }, select: { account: true } } }
                }), loggedInUser);
